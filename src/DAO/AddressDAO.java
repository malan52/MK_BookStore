...
public class AddressDAO {
	...
	public AddressBean retrieveBillingAddr(String username) throws SQLException {
		String query = "SELECT * from ADDRESS where USERNAME = '" + username + "' AND ADDRTYPE = 'Billing'";//only one billing address for one user
	...
	public Map<Integer, AddressBean> retrieveShippingAddr(String username) throws SQLException {
		String query = "SELECT * from ADDRESS where USERNAME = '" + username + "' AND ADDRTYPE = 'Shipping'";//one user may have more than one shipping address
	...
	public int updateAddr(AddressBean addr) throws SQLException {
		if (addr.getType().equals("Billing")) {//if addr is billing address, one billing for one user
			String query;
			if (retrieveBillingAddr(addr.getUsername()) == null) {// first enter billing
				...
				query = "UPDATE Address SET \"STREET\"='" + addr.getStreet() + "', \"PROVINCE\"='" + addr.getProvince()
						+ "', \"COUNTRY\"='" + addr.getCountry() + "', \"ZIP\"='" + addr.getZip() + "', \"PHONE\"='"
						+ addr.getPhone() + "' WHERE A_ID=" + a_id;
			}
			Connection con = this.ds.getConnection();
			PreparedStatement p = con.prepareStatement(query);
			p.executeUpdate();
			...
			int a_id = this.retrieveAddrByBean(addr);
			if (a_id == 0) {// addr not exist
